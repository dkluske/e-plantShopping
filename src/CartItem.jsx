...
const CartItem = ({onContinueShopping}) => {
    ...
    const handleIncrement = (item) => {
        ...
        if (foundItem) {
            dispatch(updateQuantity({name: foundItem.name, quantity: foundItem.quantity + 1}));
        }
    };
    ...
    const handleDecrement = (item) => {
        const foundItem = cart.find(cartItem => cartItem.name === item.name);
        if (foundItem) {
            dispatch(updateQuantity({name: foundItem.name, quantity: Math.max(foundItem.quantity - 1, 1)}));
        }
    };
    ...
    const handleCheckout = () => {
        alert("Thank you for your interest in our products. This functionality is not yet available. Please check back soon for updates.");
    }
